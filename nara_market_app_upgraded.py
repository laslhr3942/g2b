...
bid_api_key = "입찰공고_API_인증키"
spec_api_key = "사전규격_API_인증키"

※ 공공데이터포털(data.go.kr)에서 각 API마다 별도의 키를 발급받아야 합니다.
   - 나라장터 입찰정보 조회 서비스 → bid_api_key
   - 나라장터 입찰정보 사전규격 조회 서비스 → spec_api_key
"""
...
def get_api_key(search_type: str) -> Optional[str]:
    """Secrets에서 검색 유형에 맞는 API 키 가져오기"""
    key_mapping = {
        '입찰공고': 'bid_api_key',
        '사전규격': 'spec_api_key'
    }
    
    key_name = key_mapping.get(search_type)
    
    try:
        return st.secrets[key_name]
    except KeyError:
        st.error(f"🚨 '{search_type}' API 키가 설정되지 않았습니다.")
        st.info(f"[Settings] > [Secrets]에 '{key_name}'를 추가해주세요.")
        st.code(f'{key_name} = "여기에_API_키_입력"', language='toml')
        return None
...
def main():
    ...
    with st.sidebar:
        ...
        with st.expander("🔑 API 키 상태", expanded=False):
            bid_key_status = "✅ 설정됨" if "bid_api_key" in st.secrets else "❌ 미설정"
            spec_key_status = "✅ 설정됨" if "spec_api_key" in st.secrets else "❌ 미설정"
            st.write(f"입찰공고: {bid_key_status}")
            st.write(f"사전규격: {spec_key_status}")
    ...
    if search_btn:
        ...
        # API 키 검증
        service_key = get_api_key(search_type)
        if not service_key:
            return
